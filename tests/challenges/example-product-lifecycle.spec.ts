 ...
 */
// These tests are skipped as they will be refactored in the challenge
/* eslint-disable playwright/no-skipped-test */
test.describe.skip('Product Lifecycle - Example', () => {
  ...
  test('Complete product lifecycle: Create → Edit → Adjust Stock → Delete', async ({
    page,
  }) => {
    ...
    await test.step('Create new product', async () => {
      ...
      await page
        ...
        .selectOption(testProduct.category);
      await page
        .getByTestId('low-stock-threshold-input')
        .fill(testProduct.lowStockThreshold.toString());
    ...
    await test.step('Adjust product stock', async () => {
      ...
      const adjustButton = page
        .locator(`[data-testid^="adjust-stock-"]`)
        .first();
    ...
    await test.step('Delete product', async () => {
      ...
      const deleteButton = page
        .locator(`[data-testid^="delete-product-"]`)
        .first();
  ...
  test('Verify low stock alerts', async ({ page }) => {
    await test.step('Create product with low stock', async () => {
      ...
      await page
        ...
        .fill('Product with low inventory');
    ...
    await test.step('Verify low stock indicators', async () => {
      // Check products page
      await page.goto('/products');
      const stockBadge = page
        .locator('[data-testid^="product-row-"]')
        .filter({ hasText: 'Low Stock Product' })
        .locator('.bg-red-100');
  ...
  test('Data-driven product creation from test data', async ({ page }) => {
    ...
    for (const product of testData.validProducts) {
      await test.step(`Create product: ${product.name}`, async () => {
        ...
        await page
          .getByTestId('low-stock-threshold-input')
          .fill(product.lowStockThreshold.toString());
...
// These tests are skipped as they will be refactored in the challenge
/* eslint-disable playwright/no-skipped-test */
test.describe.skip('Form Validation - Negative Tests', () => {
  test.beforeEach(async ({ page }) => {
    await resetApplicationData(page);
    await loginAsAdmin(page);
    await page.goto('/products/new');
  ...
  test('Should validate required fields', async ({ page }) => {
    ...
    await expect(page.getByTestId('sku-error')).toContainText(
      'SKU is required'
    );
    await expect(page.getByTestId('name-error')).toContainText(
      'Name is required'
    );
    await expect(page.getByTestId('price-error')).toContainText(
      'Price is required'
    );
    await expect(page.getByTestId('stock-error')).toContainText(
      'Stock is required'
    );
  });
